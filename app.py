...
def launch_instance():
    if request.method == 'POST' and request.form['submit'] == 'Create instance':
        ...
        if add_jenkins:
            user_data +='docker run -p 8080:8080 -p 50000:50000 -d -v /var/run/docker.sock:/var/run/docker.sock -v jenkins_home:/var/jenkins_home jenkins/jenkins:lts'
...
def create_jenkins_job():
    if request.method == "POST":
        ...
        server = jenkins.Jenkins('http://54.221.34.0:8080/', username='sivan_marom', password='1234')
