...
def launch_instance():
    if request.method == 'POST' and request.form['submit'] == 'Create instance':
        ...
        if add_jenkins:
            ...
            user_data += 'docker exec -u root jenkins_master bash -c "curl https://get.docker.com/ > dockerinstall && chmod 777 dockerinstall && ./dockerinstall" && chmod 777 /var/run/docker.sock\n'
...
def create_jenkins_job():
    if request.method == "POST":
        ...
        server = jenkins.Jenkins('http://34.207.123.77:8080/', username='sivan_marom', password='1234')
