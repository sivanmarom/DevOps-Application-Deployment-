...
class Test_class(BaseClass):
    def test_signup(self,setup):
        ...
        log = self.log_conf()
        driver = setup
        driver.get("http://204.236.214.151:5000/")
        sign_up = driver.find_element(By.CSS_SELECTOR, ".register_link")
        sign_up.click()
        name = 'sivan'
        user_name = driver.find_element(By.CSS_SELECTOR, 'input[placeholder="Enter name"]').send_keys(name)
        password = driver.find_element(By.CSS_SELECTOR,'input[placeholder="Enter password"]').send_keys('1234')
